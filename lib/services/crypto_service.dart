...
class CryptoService {
    ...
    CryptoService.forDecryption({
        ...
        if (uri == null || userPassword == null) {
            throw ArgumentError('URI and userPassword must be provided for decryption.');
        }
    }

    CryptoService() : uri = null, userPassword = null;
    ...
    Future<SecretKey> _generateArgon2idKeyFromUri() async {
        if (uri == null || userPassword == null) {
            throw StateError('_generateArgon2idKeyFromUri called in invalid context.');
        }
        final argon2 = Argon2id(
            parallelism: 1,      
            memory: 16,          
    ...
    Future<Uint8List> _decryptContent(SecretKey decryptionKey) async {
        if (uri == null) {
            throw StateError('_decryptContent called in invalid context (uri is null).');
        }
